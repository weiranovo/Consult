...
const submit = async () => {
  ...
  loading.value = true
  const res = await createConsultOrder(store.state.consult)
  orderId.value = res.data.id
  loading.value = false
  store.commit('consult/clear')
  // 打开
  show.value = true
}
...
const loadData = async () => {
  const res = await getConsultOrderPre({
    type: store.state.consult.type,
    illnessType: store.state.consult.illnessType
  })
  payInfo.value = res.data
  // 设置默认优惠券
  store.commit('consult/setCoupon',payInfo.value.couponId)

}

const patient = ref<Patient>()
const loadPatient = async () => {
  if (!store.state.consult.patientId) return
  const res = await getPatientDetail(store.state.consult.patientId)
...
const pay = async () => {
  ...
  console.log(orderId.value,paymentMethod.value)
  console.log(orderId.value,paymentMethod.value)
...
onMounted(() => {
  ...
  const valid = validKeys.every((key) => store.state.consult[key] !== undefined)
  if (!valid) {
    return showDialog({
      title: '温馨提示',
      message:
...
<template>
  <div class="consult-pay-page" v-if="payInfo && patient">
    ...
    <van-cell-group>
      ...
      ></van-cell>
      <van-cell title="病情描述" :label="store.state.consult.illnessDesc"></van-cell>
    </van-cell-group>
